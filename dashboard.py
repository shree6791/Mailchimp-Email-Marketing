"""Streamlit dashboard entrypoint. Run: ``streamlit run dashboard.py`` (do not name this file ``streamlit.py`` — it would shadow the Streamlit library)."""
...
from src.serving.streamlit.data_loading import dashboard_api_base, load_trend_dashboard_data
from src.serving.streamlit.theme import inject_mailchimp_theme


@st.cache_data
def load_data(api_base: str | None) -> tuple[pd.DataFrame, pd.DataFrame]:
    return load_trend_dashboard_data(api_base=api_base)
...
def main() -> None:
    ...
    try:
        topic_insights, _ = load_data(api_base)
    ...
    if topic_insights.empty:
        st.warning("No topic insights found (run the pipeline or check the API).")
        st.stop()
